...
import { NEPAL_DISTRICTS } from '@/shared/constants';
...
export const RegisterForm = ({ onSubmit, loading, apiErrors, onClearErrors }) => {
    ...
    // District options from constants
    const districtOptions = NEPAL_DISTRICTS.map(district => ({
    ...
    const handleFieldChange = (field, value) => {
        ...
        if (field === 'username') {
            ...
            if (error) setFieldErrors(prev => ({ ...prev, username: error }));
            else setFieldErrors(prev => { const n = { ...prev }; delete n.username; return n; });
        }
        if (field === 'email') {
            const error = validateEmail(value);
            if (error) setFieldErrors(prev => ({ ...prev, email: error }));
            else setFieldErrors(prev => { const n = { ...prev }; delete n.email; return n; });
        }
        if (field === 'password') {
            const error = validatePassword(value);
            if (error) setFieldErrors(prev => ({ ...prev, password: error }));
            else setFieldErrors(prev => { const n = { ...prev }; delete n.password; return n; });
            if (form.password2) {
                const confirmError = validateConfirmPassword(value, form.password2);
                if (confirmError) setFieldErrors(prev => ({ ...prev, password2: confirmError }));
                else setFieldErrors(prev => { const n = { ...prev }; delete n.password2; return n; });
            }
        }
        if (field === 'password2') {
            const error = validateConfirmPassword(form.password, value);
            if (error) setFieldErrors(prev => ({ ...prev, password2: error }));
            else setFieldErrors(prev => { const n = { ...prev }; delete n.password2; return n; });
        }
        if (field === 'phone') {
            const error = validatePhone(value);
            if (error) setFieldErrors(prev => ({ ...prev, phone: error }));
            else setFieldErrors(prev => { const n = { ...prev }; delete n.phone; return n; });
        }
        if (field === 'date_of_birth') {
            const error = validateAge(value);
            if (error) setFieldErrors(prev => ({ ...prev, date_of_birth: error }));
            else setFieldErrors(prev => { const n = { ...prev }; delete n.date_of_birth; return n; });
        }
    ...
    const handleSubmit = async (e) => {
        ...
        if (confirmPasswordError) newFieldErrors.password2 = confirmPasswordError;
        ...
        if (dateOfBirthError) newFieldErrors.date_of_birth = dateOfBirthError;
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                    ...
                    <Input 
                        ...
                        className={`rounded-xl ${fieldErrors.first_name ? 'border-destructive' : ''}`}
                    />
                ...
                <div className="space-y-2">
                    ...
                    <Input 
                        ...
                        className={`rounded-xl ${fieldErrors.last_name ? 'border-destructive' : ''}`}
                    />
            ...
            <div className="space-y-2">
                ...
                <div className="flex gap-2">
                    <div className="flex-1">
                        <Input 
                            ...
                            className={`rounded-xl ${fieldErrors.username || apiErrors?.username ? 'border-destructive' : ''}`}
                        />
                    </div>
                    <Button 
                        type="button" 
                        ...
                        className="rounded-xl px-4 border-emerald-200 hover:bg-emerald-50 hover:text-emerald-600" 
                ...
                {apiErrors?.username && !fieldErrors.username && (
                    <p className="text-xs text-destructive flex items-center gap-1 mt-1">
            ...
            <div className="space-y-2">
                ...
                <Input 
                    ...
                    className={`rounded-xl ${fieldErrors.email || apiErrors?.email ? 'border-destructive' : ''}`}
                />
                {fieldErrors.email && (
                    <p className="text-xs text-destructive flex items-center gap-1 mt-1">
                ...
                {apiErrors?.email && !fieldErrors.email && (
                    <p className="text-xs text-destructive flex items-center gap-1 mt-1">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                    ...
                    <Input 
                        ...
                        className={`rounded-xl ${fieldErrors.phone || apiErrors?.phone ? 'border-destructive' : ''}`}
                    />
                    {fieldErrors.phone && (
                        <p className="text-xs text-destructive flex items-center gap-1 mt-1">
                    ...
                    {apiErrors?.phone && !fieldErrors.phone && (
                        <p className="text-xs text-destructive flex items-center gap-1 mt-1">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                    ...
                    <Input 
                        ...
                        required 
                        className={`rounded-xl ${fieldErrors.date_of_birth ? 'border-destructive' : ''}`}
                    />
                ...
                <div className="space-y-2">
                    ...
                    {fieldErrors.geographical_region && (
                        <p className="text-xs text-destructive flex items-center gap-1 mt-1">
            ...
            {/* District & Location - District is now a dropdown */}
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                    <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">
                        {t('auth.district')} *
                    </Label>
                    <Select value={form.district} onValueChange={v => handleFieldChange('district', v)}>
                        <SelectTrigger className={`rounded-xl border-border/60 focus:border-emerald-500 ${fieldErrors.district ? 'border-destructive' : ''}`}>
            ...
            <div className="space-y-2">
                ...
                <div className="relative">
                    <Input 
                        ...
                        minLength={8} 
                        placeholder="••••••••" 
                        className={`rounded-xl pr-10 ${fieldErrors.password ? 'border-destructive' : ''}`}
                    />
                    <button 
                        type="button" 
                ...
                {form.password && !fieldErrors.password && (
                    <div className="space-y-1 mt-1">
                        <div className="flex gap-1">
                            {[1, 2, 3, 4, 5].map((level) => (
                                <div key={level} className={`h-1 flex-1 rounded-full transition-all ${level <= passwordStrength.score ? passwordStrength.color : 'bg-gray-200'}`}/>
                            ))}
                        </div>
            ...
            <div className="space-y-2">
                ...
                <div className="relative">
                    <Input 
                        ...
                        minLength={8} 
                        placeholder="••••••••" 
                        className={`rounded-xl pr-10 ${fieldErrors.password2 ? 'border-destructive' : ''}`}
                    />
                    <button 
                        type="button" 
                ...
                {form.password && form.password2 && !fieldErrors.password2 && form.password === form.password2 && (
            ...
            <div className="mt-4 pt-2">
                <div className="flex items-start gap-3 p-4 bg-muted/20 rounded-xl border border-border/60">
                    <input 
                        type="checkbox" 
                        id="agreedToAll" 
                        checked={form.agreed_to_terms && form.agreed_to_privacy} 
                        onChange={(e) => {
            ...
            <Button 
                type="submit" 
                className="w-full h-12 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-base font-bold mt-4" 
                disabled={loading || !form.agreed_to_terms || !form.agreed_to_privacy || Object.keys(fieldErrors).length > 0}
            >
