...
import { Wheat, Calendar, Info, ChevronLeft, Droplets, Sprout, Trash2, Edit2, AlertCircle, MapPin, Mountain, Archive, RefreshCw } from 'lucide-react';
...
const CropDetailPage = () => {
    ...
    const validateName = (name) => {
        if (!name || name.trim() === '')
            return `${t('crops.cropName')} ${t('common.isRequired')}`;
        if (name.length < 2)
    ...
    const validateFieldName = (fieldName) => {
        if (!fieldName || fieldName.trim() === '')
            return `${t('crops.fieldName')} ${t('common.isRequired')}`;
    ...
    const validateArea = (area) => {
        if (!area)
            return `${t('crops.area')} ${t('common.isRequired')}`;
    ...
    const validateDate = (date) => {
        if (!date)
            return `${t('crops.plantingDate')} ${t('common.isRequired')}`;
    ...
    const getStatusMessage = () => {
        if (isActive)
            return t('crops.statusActive');
        if (isHarvested)
            return t('crops.statusHarvested');
    ...
    if (loading)
        return (<MainLayout title={t('crops.detailTitle')} subtitle="...">
    ...
    if (!crop)
        return (<MainLayout title={t('crops.detailTitle')} subtitle="Not Found">
      <div className="text-center py-20">
        <p>{t('crops.notFound')}</p>
        <Button onClick={() => navigate('/crops')} className="mt-4">{t('common.back')}</Button>
      </div>
    </MainLayout>);
    return (<MainLayout title={language === 'np' && crop.name_np ? crop.name_np : crop.name} subtitle={crop.variety || t('crops.localVariety')}>
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2">
            ...
            {canEdit && (<Button variant="outline" size="sm" onClick={openEditDialog} className="rounded-xl gap-2">
                <Edit2 className="w-4 h-4"/> {t('common.edit')}
              </Button>)}
            
            {/* Status Change Button */}
            {canChangeStatus && (<Button variant="outline" size="sm" onClick={() => {
            ...
            {/* Delete Button */}
            {canDelete && (<Button variant="outline" size="sm" onClick={() => setShowDeleteConfirm(true)} className="rounded-xl gap-2 text-red-600 hover:text-red-700 hover:bg-red-50">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-8">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-lg sm:text-xl font-bold capitalize">{crop.growth_stage}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <ResponsiveDialog isOpen={showStatusDialog} onClose={() => setShowStatusDialog(false)} title={t('common.changeStatus')} maxWidth="sm">
        <div className="space-y-4">
          <div className="text-center">
            ...
            {newStatus === 'harvested' && (<p className="text-sm text-amber-600 bg-amber-50 p-3 rounded-lg">
                ⚠️ {t('crops.harvestedWarning')}
              </p>)}
            {newStatus === 'done' && (<p className="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">
                📄 {t('crops.doneWarning')}
              </p>)}
          </div>
          <div className="flex gap-3">
            <Button onClick={handleStatusChange} disabled={isSubmitting} className="flex-1 bg-green-600 hover:bg-green-700">
              {isSubmitting
            ? t('common.saving')
            : (newStatus === 'harvested' ? t('crops.markAsHarvested') : t('common.markAsDone'))}
            </Button>
      ...
      <ResponsiveDialog isOpen={showEditDialog} onClose={() => { setShowEditDialog(false); setEditErrors({}); }} title={`${t('common.edit')} ${t('crops.title')}`} maxWidth="md">
        <div className="space-y-4 max-h-[70vh] overflow-y-auto px-1">
          ...
          <div className="space-y-1.5">
            ...
            {editErrors.field_name && (<p className="text-xs text-red-500 flex items-center gap-1 mt-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              {editErrors.field_area && (<p className="text-xs text-red-500 flex items-center gap-1 mt-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              {editErrors.planting_date && (<p className="text-xs text-red-500 flex items-center gap-1 mt-1">
          ...
          {hasErrors && (<div className="p-3 bg-yellow-50 rounded-lg border border-yellow-200">
