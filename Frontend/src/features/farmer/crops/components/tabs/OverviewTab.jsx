...
import { Wheat, Calendar, Droplets, MapPin, Info, Sprout, DollarSign, AlertTriangle, Clock, CircleCheck, RotateCcw, RefreshCw, ChevronLeft, ChevronRight, Filter, X } from 'lucide-react';
...
function RelatedAlerts({ itemId, source }) {
    ...
    // Instead, keep the current page if it's valid, otherwise go to last page
    useEffect(() => {
        ...
        <div className="farm-card p-6 border-rose-100 bg-rose-50/10 rounded-2xl shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4 p-3 bg-gray-50 rounded-lg border border-gray-100">
                ...
                <div className="ml-auto text-xs text-muted-foreground">
                    {language === 'np' 
                        ? `${filteredAlerts.length} मध्ये ${totalItems} फेला पर्यो`
                        : `Found ${totalItems} of ${alerts.length}`}
                </div>
            </div>
...
export function OverviewTab({ crop }) {
    ...
    return (<div className="space-y-6">
      ...
      <RelatedAlerts itemId={crop.id} source="crop" />
